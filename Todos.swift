...
class Todos: UITableViewController {
    ...
    @IBAction func makeListPrivate(sender: UIBarButtonItem) {
        ...
        if let userId = Meteor.client.userId() {
            ...
            if let index = lists.indexOf({ list in return list._id == listId }) {
             //if object._id == listId {
                var object = lists[index]
                if let objectUserId = object.userId where (objectUserId == userId) {
                    
                    object.userId = ""
                    //let fields = ["userId":"true"]
                    //let operation = ["$unset":fields]
                    lists[index] = object
                    //lists.update(object, withMongoOperation: operation)
